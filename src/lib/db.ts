...
async function topUpGameItems(c: Client): Promise<void> {
  ...
  const items: [string, string, "human" | "ai", string | null, string][] = [
    ...
    // ==== 3차 확장 풀: 목소리 다양화 ====
    // 앞선 AI 문항이 전부 "존댓말+클리셰+이모지" 한 가지 톤이라 그 패턴만 외우면
    // 내용을 안 읽고도 맞힐 수 있었다. 아래는 서로 다른 AI 말투(리스트형, 캐주얼
    // 반말, 블로그 후킹, 과잉 헤지, 과다 느낌표, 번역투, 사무형)로 흩어 놓고,
    // 반대로 사람 쪽에도 존댓말·형식체 항목을 섞어 "말투=정답"이라는 지름길을
    // 막는다.
    ["설명", "여행 전 체크리스트입니다. 1. 여권 유효기간 확인 2. 유심은 출국 전 미리 구매 3. 환전은 공항보다 시내 은행이 유리합니다.", "ai", "리스트형AI풍", "클리셰 없이도 번호 나열 자체가 지문. 실패담·예외 없이 매끈함."],
    ["SNS", "오늘 진짜 럭키비키한 하루였다! 작은 행운도 소중하게 느껴지는 하루였음ㅎㅎ", "ai", "캐주얼AI풍", "반말·유행어를 흉내 내지만 정작 무슨 일이 있었는지는 끝까지 안 나옴."],
    ["설명", "다이어트 성공 필수 팁 5가지: 첫째 식단 관리, 둘째 규칙적 운동, 셋째 충분한 수면, 넷째 스트레스 관리, 다섯째 꾸준함. 지금 바로 시작해보세요!", "ai", "블로그SEO풍", "제목형 리스트 + 마지막 CTA, 블로그 후킹 문법 그대로."],
    ["Q&A", "정확한 진단을 위해서는 전문가와의 상담이 필요할 수 있습니다. 아래 내용은 일반적인 정보이며 개인의 상황에 따라 다를 수 있습니다.", "ai", "헤지과잉AI풍", "책임 회피성 캐비엇만 있고 실제 답은 끝까지 없음."],
    ["리뷰", "이 제품 정말 최고예요!!! 강력 추천합니다!!! 후회 안 하실 거예요!!!", "ai", "열정과다AI풍", "느낌표 반복으로 열정을 흉내 내지만 구체적 사용 경험은 0."],
    ["뉴스", "해당 발표에 따르면, 관련된 여러 요인들이 복합적으로 작용한 것으로 확인되었다고 전해졌습니다.", "ai", "번역투AI풍", "이중 피동('전해졌다고')과 번역투 어순, 출처 없는 '복합적 요인'."],
    ["Q&A", "네, 확인 후 회신드리겠습니다. 추가 문의 사항 있으시면 말씀해 주세요.", "ai", "사무형AI풍", "사무적으로 완벽하지만 구체적 기한·상황 언급이 전혀 없음."],
    ["에세이", "우리는 모두 각자의 속도로 걸어가고 있습니다. 남과 비교하지 않아도 괜찮습니다.", "ai", "위로격언AI풍", "반박 불가능한 위로 명제, 화자의 실제 사연은 하나도 없음."],
    ["Q&A", "그냥 재설치하면 대부분 해결됩니다! 그래도 안 되면 캐시 삭제도 한번 해보세요~", "ai", "캐주얼AI풍", "반말체를 흉내 내지만 '대부분'이라는 통계적 표현과 본인 경험 부재가 티남."],
    ["설명", "성공적인 발표를 위한 핵심은 명확한 구조, 적절한 시간 배분, 그리고 청중과의 눈맞춤입니다.", "ai", "리스트형AI풍", "삼단 나열형 조언, 저자 본인이 발표를 해본 흔적이 없음."],

    ["Q&A", "안녕하세요, 문의주신 건 확인했습니다. 죄송하지만 이번 주는 재고가 없어서 다음 주 화요일에 입고 예정입니다. 번거로우시겠지만 그때 다시 연락 주시면 감사하겠습니다.", "human", null, "존댓말이지만 구체적 요일(화요일)과 재고 상황 등 실무 디테일이 있음."],
    ["리뷰", "정중히 말씀드리면, 배송은 빨랐으나 설명서에 오탈자가 세 군데 있어 조립하며 헷갈렸습니다. 그래도 as 문의드리니 바로 답변 주셔서 좋았습니다.", "human", null, "격식체지만 오탈자 '세 군데'처럼 사람만 셀 법한 구체 수치."],
    ["SNS", "오늘 회사 워크숍에서 사장님이 갑자기 마이크 잡고 3절까지 부르셔서 다들 당황했습니다ㅋㅋ", "human", null, "존댓말 속에 섞인 ㅋㅋ와 구체적 사건(3절까지)."],
    ["에세이", "인생은 마라톤이라고들 하지만, 저는 5분마다 편의점 들르고 싶어지는 마라톤이라고 생각합니다.", "human", null, "격언을 비틀어 자기 유머로 바꾸는 태도, 존댓말이어도 위트가 개인적."],
    ["설명", "이 앱 설정법 알려드릴게요. 저도 처음엔 헷갈렸는데, 알림 끄기가 자꾸 안 먹혀서 결국 껐다 켰더니 됐어요.", "human", null, "정중한 어투 안에 본인의 시행착오가 그대로 드러남."],
    ["Q&A", "네 확인해보겠습니다! 근데 저도 지금 재택이라 컴퓨터 앞이 아니라서 30분만 기다려주시겠어요?", "human", null, "존댓말이지만 재택·30분 같은 즉흥적 실제 제약이 있음."],
    ["리뷰", "이거 완전 대박!!! 근데 배송이 3일이나 늦어서 마음 졸였음... 그래도 물건 오니까 다 용서됨.", "human", null, "느낌표로 열정 표현하되, 구체적 지연 일수와 감정 반전이 있음."],
    ["SNS", "고객센터랑 통화 40분 했는데 결국 해결 못 함. 현타 옴.", "human", null, "사무적 단어(고객센터)와 구체 시간(40분), 신조어(현타)의 조합."],
    ["Q&A", "말씀하신 부분 제가 놓쳤네요. 죄송합니다, 오늘 중으로 다시 정리해서 보내드리겠습니다.", "human", null, "실수를 인정하는 존댓말, 사람만 하는 즉흥적 사과."],
    ["설명", "회의실 예약 시스템 쓰는 법인데요, 저도 두 번 겹치게 잡아서 민망했던 적 있어서 알려드려요. 시작 10분 전엔 꼭 확인하세요.", "human", null, "본인의 실수담을 근거로 조언, 정중체와 개인 일화가 공존."],
  ];
...
async function seedChallenges(c: Client): Promise<void> {
  ...
  const prompts: [string, string, string][] = [
    ...
    // 아래는 위와 다른 말투(캐주얼 반말, 리스트형, 과다 느낌표)로 흩어, 답변
    // 말투 하나만 외워서 맞히는 걸 막는다.
    ["카톡", "친구에게 생일 축하 카톡을 써주세요.", "생일 축하해!! 오늘 하루 완전 행복한 일만 가득하길 바라!! 이따 케이크 사갈게 ㅎㅎ"],
    ["자기소개", "동아리 첫 모임에서 할 자기소개 한두 문장을 써주세요.", "안녕하세요! 저는 새로운 것에 도전하는 걸 좋아하고, 사람들과 어울리는 시간을 소중히 여기는 사람입니다. 잘 부탁드려요!"],
    ["댓글", "맛집 후기 게시물에 달 댓글을 하나 써주세요.", "우와 진짜 맛있어 보여요!! 저도 다음 주에 꼭 가보고 싶네요! 후기 감사합니다 :)"],
  ];
